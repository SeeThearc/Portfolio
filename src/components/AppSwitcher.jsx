...
function Panel({name,children,onClose,className=''}) {
 ...
 return <dialog ref={ref} className={`os-panel ${className}`} aria-label={name} onCancel={onClose} onClick={e=>{if(e.target===ref.current)onClose()}}><div className="os-panel-heading"><h2>{name}</h2><button onClick={onClose} aria-label={`Close ${name}`}><NavigationIcon close /></button></div>{children}</dialog>
}
...
const apps=[{label:'Home',path:'/home',icon:'⌂',color:'#eb825d'},{label:'Projects',path:'/projects',icon:'⊞',color:'#468cf0'},{label:'About Me',path:'/about',icon:'◉',color:'#a18be0'},{label:'Experience',path:'/experience',icon:'▣',color:'#63b395'},{label:'Skills',path:'/skills',icon:'ϟ',color:'#e3aa4c'},{label:'Contact',path:'/contact',icon:'✉',color:'#9980d7'}]
function loadRecent(){try{return JSON.parse(sessionStorage.getItem('ipad-recents')||'[]').map(p=>p==='/about?tab=Skills'?'/skills':p==='/about?tab=Experience'?'/experience':p).filter(p=>apps.some(a=>a.path===p))}catch{return []}}
export default function AppSwitcher({children}) {
 const navigate=useNavigate(),location=useLocation(),{theme,setTheme,themes}=useTheme(),{isMuted,isAudioBusy,toggleMute}=useAudio()
