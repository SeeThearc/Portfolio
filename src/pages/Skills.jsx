...
export default function Skills() {
  ...
  return <AppWindow name="Skills" icon="ϟ" subtitle="My developer toolbox" className="skills-app" actions={<Link to="/projects">See my work ↗</Link>}>
    <div className="portfolio-scroll">
      ...
      <div className="skill-category-grid">{visible.map(g => <section className="skill-category" key={g.name} style={{"--skill-accent": {Languages:"#f0cf8a","Web development":"#9ec9ff","AI & machine learning":"#d4b7ff",Blockchain:"#8fdec6",Foundations:"#f0b6a6"}[g.name]}}><div className="skill-category-heading"><span>{g.icon}</span><div><h3>{g.name}</h3><p>{g.description}</p></div><small>{g.items.length}</small></div><div className="skill-token-list">{g.items.map(skill => <span key={skill}>{skill}</span>)}</div></section>)}</div>
